...
output(" `i".$info['specname']."`i");
	...
	*/
	...
	if ($_GET[action]=="sell"){
		...
		item_show_invent(' (spellshop = 2 OR spellshop = 3) AND owner='.$session['user']['acctid'], false, 2, 1, 1, '`vDu hast keine Zauber, die du dem Alten anbieten könntest.');
		
		addnav('Zurück');			
		addnav("Zum Laden","academy.php?op1=bringmetolife");
	}else if ($_GET[action]=="buy"){ // ok, water's getting cold ^^
						
		output("`v\"`RDu willst Magie nutzen, ohne sie mühsam studieren zu müssen? Dann bist du hier genau richtig.`v\" Mit diesen Worten überreicht dir der Alte eine Liste aller Zauber, die er dir anbieten kann. \"`RBitte sehr. Wähle dir etwas aus.".($rebate?" Achja, dank deiner Gildenmitgliedschaft gewähre ich dir `^".$rebate." %`R Rabatt!":"")."`v\"`n`n");
		
		$rebate = (100 - $rebate) * 0.01;
		
		item_show_invent(' (spellshop = 1 OR spellshop = 3) ', true, 1, $rebate, $rebate, '`v"`RTut mir Leid, mein Freund. Wir haben keine Zauber für dich.`v"');
