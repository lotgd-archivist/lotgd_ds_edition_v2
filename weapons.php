...
if ($HTTP_GET_VARS[op]==""){
  output("`!MightyE `7steht hinter einem Ladentisch und scheint dir nur wenig Interesse entgegen zu bringen, als du eintrittst. ");
	...
	output("um stärkere ".($session[user]['sex']?"Frauen":"Männer")." als dich zu töten.`n`n");
...
output("MightyE lacht aus vollem Halse als du ihm entgegentrittst und wendet sich dann auch wieder seiner Arbeit zu, nachdem er etwas Unverständliches gemurmelt hat. Was immer es war, es klang nicht sehr freundlich.");
addnav("Zurück zum Marktplatz","market.php");
} else {
output("MightyE legt seine Sachen bei Seite und mustert dich eindringlich und nickt. Ihr steht euch nun gegenüber. Einige Schaulustige haben sich bereits um euch versammelt. Noch kannst du weglaufen. Der Kampf kostet dich, egal wie er ausgeht, 100 Donation Punkte und weiter 400 wenn du gewinnst.");
...
// Ende1
	...
	output(" Gold für `5".$session[user][weapon]."`# ".($rebate?"und `^".$rebate." %`# Rabatt dank deiner Gildenmitgliedschaft.":"").". Klicke einfach auf die Waffe, die du kaufen willst... was auch immer 'klick' bedeuten mag`7.\". ");
...
if ($battle){
                ...
                addnav("Zurück zum Marktplatz","market.php");
                    ...
                    output ("`7Anstatt dich in das Reich des Schlafes zu befördern reicht `#MightyE`7 dir eine Hand und hilft dir auf. Das war wohl nichts!");
                    $session[user][hitpoints]=$session[user][maxhitpoints];
                    $battle=false;
