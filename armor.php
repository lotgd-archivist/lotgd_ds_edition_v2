...
if ($HTTP_GET_VARS[op]==""){
	...
	output("ihre Waren durchstöberst, blickt sie auf dein(e/n) ".$session[user][armor]." und bietet dir dafür im Tausch `^$tradeinvalue`5 Gold ".($rebate?"und einen Rabatt in Höhe von `^".$rebate." %`5 dank deiner Gildenmitgliedschaft":"")." an.");
			...
			if ($session['user']['reputation']<=-10){
				...
				if ($klau==1){ // Fall nur für Diebe
					...
					if ($session[user][goldinbank]<0){
						...
						output("Als du dein(e/n) `%$row[armorname]`5 stolz auf dem Marktplatz präsentierst, packt dich von hinten `!MightyE`5's starke Hand. Er entreißt dir $row[armorname] gewaltsam, ");
...
if(is_array($arr_arm)) {
	...
	$int_aid = item_add($session['user']['acctid'],'rstdummy',true,$arr_arm);
