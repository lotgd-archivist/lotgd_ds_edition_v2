...
else if ($_GET['op']=="transfer")
{
    ...
    if ($session[user][goldinbank]>=0)
    {
        ...
        if ($rowe['goldout'] > 0)
        {
            output("(Du hast heute schon `^{$rowe['goldout']}
            `6 Gold überwiesen.)`n`n");
        }
        else
        {
            output("`n`n");
...
else if ($_GET['op']=="transfer2")
{
    ...
    if (db_num_rows($result)==1)
    {
        ...
        output("<input type='hidden' name='to' value='".$row['acctid']."'><input type='hidden' name='amount' value='$amt'><input type='submit' class='button' value='Überweisung abschließen'></form>",true);
...
else if ($_GET['op']=="transfer3")
{
    ...
    else
    {
        ...
        if (db_num_rows($result)==1)
        {
            ...
            {
                output("`6Die Überweisung wurde nicht durchgeführt: `&{$row['name']}
                `6 darf maximal `^$maxtfer`6 Gold empfangen.");
            }
            else if ($row['goldin']>=getsetting("transferreceive",3))
            {
                output("`&{$row['name']}
                `6 hat heute schon zu viele Überweisungen erhalten. Du wirst bis morgen warten müssen.");
            }
            else if ($amt<$minfer)
            {
            ...
            else
            {
                ...
                `6 hat dir `^$amt`6 Gold auf dein Konto überwiesen!");
            }
        }
        else
        {
...
else if ($_GET['op']=="gemtrans2")
{
    ...
    if (db_num_rows($result)==1)
    {
        ...
        output("<input type='hidden' name='to' value='".$row['acctid']."'><input type='submit' class='button' value='Versand abschließen'></form>",true);
...
else if ($_GET['op']=="gemtrans3")
{
    ...
    if (db_num_rows($result)==1)
    {
        ...
        {
            output("`&{$row['name']}
            `6 kann noch keine Edelsteine in Empfang nehmen. Der Empfänger muss mindestens Level ".getsetting('bankgemtrflvl',3)." sein.");
        }
        else if ($row['acctid']==$session['user']['acctid'])
        {
            output("`6Du kannst dir nicht selbst einen Edelstein schenken. Das macht keinen Sinn!");
        }
        else if ($row['gemsin']>=getsetting("bankmaxgemstrf",4))
        {
            output("`&{$row['name']}
            `6 hat heute schon zu viele Edelsteine erhalten. Du wirst bis morgen warten müssen.");
        }
        else if (ac_check($row))
        {
        ...
        else
        {
            ...
            debuglog('Versandte einen Edelstein an ',$row['acctid']);
            
			user_set_aei(array('gemsout'=>$rowe['gemsout']+1));
				
			user_set_aei(array('gemsin'=>$row['gemsin']+1),$row['acctid']);
			
            $sql = "UPDATE accounts SET gems=gems+1 WHERE acctid='{$row['acctid']}'";
            db_query($sql);
            output("`6Versand erfolgreich!");
            systemmail($row['acctid'],"`#Du hast einen Edelstein geschenkt bekommen!`0","`&{$session['user']['name']}
            `6 war so freundlich und hat dir `#1 Edelstein`6 geschenkt!");
        }
    }
    else
    {
