...
if ($_GET[op] == "wkkauf"){
	...
	if ($_GET[id]>"" && $_GET[pid]>""){
		...
		if ( $count ){
			...
			output("Jaja, es ist alles in Ordnung. Dein Opfer leidet bereits unter $row[tpl_name]`%. Behalte dein Geld. Einen schönen Tag noch. Und jetzt ... lass mich alleine.`!\"");
			...
			item_add( $session['user']['acctid'], 0, false, $row );
			...
			output("mit der anderen Hand deine $goldcost Gold und $row[tpl_gems] einstreicht. Dann nickt sie dir kurz zufrieden zu und weist dir den Weg zur Tür.`nDein Opfer wird an $row[tpl_name]`! eine Weile seine Freude haben.");
		...
		output("`nWähle ein Schicksal für dein Opfer:`n`n`4");
		output("<ul>",true);
	...
	if ($rowe['witch']<getsetting("witchvisits",3)){
		...
		if ($rowe['usedouthouse'] && $session[user][gems]) addnav("Druck auf die Blase (1 Edelstein)","hexe.php?op=blase");
		if ($rowe['seenbard'] && $session[user][gems]) addnav("Bardenhals befeuchten (1 Edelstein)","hexe.php?op=barde");
		if ($rowe['lottery'] && $session[user][gems]) addnav("Nochmal Lotto (1 Edelstein)","hexe.php?op=lotto");
