...
if ($session['user']['marks']<32) {
	...
	if (getsetting("limithp",0)>0 && $session[user][maxhitpoints]>$maxhp){
        ...
        debuglog("verlor $losthp permanente Lebenspunkt auf der Flucht vor dem Vampir im Wald.");
	...
	$session[user][specialinc]=""; } else
	{ 	output("Ausgehungert und vom Geruch deiner enormen Lebenskraft fast wahnsinnig überwältigt dich ein Vampir auf deiner Flucht und setzt seine Zähne an deinen Hals an.");
        output("`nDoch dann erkennt er das Zeichen des Blutgottes an deinem Hals und weicht in Ehrfurcht zurück.`n");
        output("`nFür sein grobes Auftreten entschädigt er dich mit `^1000 Goldstücken`n");
