...
if ($HTTP_GET_VARS['count']==3) {
	...
	switch($HTTP_GET_VARS[op]){
		...
		case "weather":
			...
			$count++;
			$w = get_weather();
			output("`t\"`qAlso ".$w['name'].", was hältst du von diesem Wetter?`t\"`nDer Drachen legt den Kopf schief und schaut dich an. Ein kurzes ");
		...
		case "city":
			$count=$HTTP_GET_VARS['count'];
			$count++;
			output("`t\"`qWusstest du, dass das Dorf ".getsetting('townname','Atrahor')." heißt? Ist ein ziemlich beeindruckendes Örtchen!`t\"`nDer Drachen gröhlt laut.`n\"`5Diese stinkende Stadt ist mir ");
