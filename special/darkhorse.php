...
case "bartender":
	if ($HTTP_GET_VARS['what']==""){
		...
		if($session['user']['superuser']>0)addnav("Gefühle","forest.php?op=bartender&what=emotes");
		...
		output(" (Shift und die Taste links neben Backspace), gefolgt von den Kodierungen, die du auch in deinem Profil schehen kannscht. Jedesch diescher Zeichen entspricht ");
		output("einer Farbe.`% kapiert?`0\"`n Hier kannscht du teschten:");
		...
		addnav("",$REQUEST_URI);
	}else if($HTTP_GET_VARS['what']=="emotes"){
		output("Der alte Mann stützt sich auf die Theke. \"`%Scho, du willscht alscho wischen wie Du Deine Gefühle auschdrücken kanscht?`0\"");
		output("  Du willst gerade antworten, als du gerade noch merkst, daß das eine rhetorische Frage war.  ");
		output("Er fährt fort: \"`%Um etwasch zu schagen wasch Deine Gefühle auschdrückt, muscht Du eines der folgenden Kommandosch eingeben`n`0",true);
		output("Tippe /sit , um \"{$session['user']['name']} setzt sich hin\", zu sagen.`n",true);
		output("Tippe /laugh , um \"{$session['user']['name']} lacht\", zu sagen.`n",true);
		output("Tippe /hit , um \"{$session['user']['name']} schlägt um sich\", zu sagen.`n",true);
		output("Tippe /grin , um \"{$session['user']['name']} grinst\", zu sagen.`n",true);
		output("Tippe /wave , um \"{$session['user']['name']} winkt\", zu sagen.`n",true);
		output("Tippe /jump , um \"{$session['user']['name']} springt\", zu sagen.`n",true);
		output("Tippe /no , um \"{$session['user']['name']} schüttelt den Kopf\", zu sagen.`n",true);
		output("Tippe /doh , um \"{$session['user']['name']} schlägt sich an den Kopf\", zu sagen.`n",true);
		output("Tippe /clap , um \"{$session['user']['name']} klatscht\", zu sagen.`n",true);
		output("Tippe /afk oder /away , um \"{$session['user']['name']} ist schnell weg\", zu sagen.`n",true);
		output("Tippe /gratz , um \"{$session['user']['name']} gratuliert\", zu sagen.`n",true);
		output("Tippe /bow , um \"{$session['user']['name']} verbeugt sich\", zu sagen.`n",true);
		output("Tippe /yes , um \"{$session['user']['name']} nickt\", zu sagen.`n",true);
		output("Tippe /help , um \"{$session['user']['name']} braucht hilfe\", zu sagen.`n",true);
		output("Tippe /beg , um \"{$session['user']['name']} bettelt\", zu sagen.`n",true);
		output("Tippe /cheer , um \"{$session['user']['name']} jubelt\", zu sagen.`n",true);
		output("Tippe /kneel , um \"{$session['user']['name']} kniet sich nieder\", zu sagen.`n",true);
		output("Tippe /bored um \"{$session['user']['name']} ist gelangweilt\", zu sagen.`n",true);
		output("Tippe /moan um \"{$session['user']['name']} seufzt\", zu sagen.`n",true);
		output("Tippe /roar , um \"{$session['user']['name']} brüllt\", zu sagen.`n",true);
		output("Tippe /sorry , um \"{$session['user']['name']} entschuldigt sich\", zu sagen.`n",true);
		output("`% kapiert?`0",true);
		output("`0`n`nDies kannst Du in jedem Text verwenden, den du eingibst.");
		addnav("",$REQUEST_URI);
		...
		if ($HTTP_GET_VARS[who]==""){
			...
			if ($_GET['subop']!="search"){
				...
				$sql = "SELECT name,alive,location,sex,level,laston,loggedin,login,restatlocation FROM accounts WHERE (locked=0 AND name LIKE '$search') ORDER BY level DESC";
				//output($sql);
				$result = db_query($sql) or die(db_error(LINK));
		...
		$sql="SELECT acctid,name,location,loggedin,laston,alive,housekey,activated,restatlocation FROM accounts WHERE acctid=".$HTTP_GET_VARS[who];
		$result = db_query($sql) or die(db_error(LINK));
		$row = db_fetch_assoc($result);
		...
		if ($row[location]==3) $loc="im Knascht";
		// end houses
