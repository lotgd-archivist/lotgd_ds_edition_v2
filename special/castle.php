...
/********Blacksmith********/ 
	...
	$session[user][specialinc]="castle.php"; 
	output("`#Der Waffenschmied nimmt `^".$session[user][weapon]."`# und arbeitet eine Weile daran. Bald steht er auf und gibt dir deine Waffe zurück. Sie scheint etwas schwerer zu sein, aber die Qualität scheint wesentlich besser als vorher zu sein. Zufrieden verlässt du den Laden. "); 
...
/********Healer********/ 
	...
	if ($session[user][gems]>=1 && $castleoptions['beautyshop_uses']<3){ 
		...
		$_SESSION['castle_charmgainer']++;
	}else if ($castleoptions['beautyshop_uses']>=3 || $_SESSION['castle_charmgainer']>=3) {
...
/********Guard Fight********/
	...
	if (e_rand(1,100) <95){ 
		if($session[user][turns]<1)
		{
			output("`#Eigentlich hast Du Dich auf das Waldschloss gefreut, aber Deine Füße und Dein lahmes Pferd freuen sich lieber auf den Feierabend. Du entscheidest doch lieber nicht zum Schloss zu reiten!");
