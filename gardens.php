...
// output("`b`c`\$IN ÜBERARBEITUNG`b`c`0`n");
	if ($session[user][seenlover]){
		...
		output("Du versuchst, dich in Gedanken auf einen heißen Flirt vorzubereiten, aber irgendwie bekommst du den Kopf nicht frei. Vielleicht solltest du bis morgen warten.");
		...
		for ($i=0;$i<db_num_rows($result);$i++){
			...
			if ($row[charisma]<$session[user][charisma]) $flirtnum=$row[charisma];
	  		output("<tr valign='top' class='".($i%2?"trlight":"trdark")."'><td></td><td>$row[name]</td><td>$row[level]</td><td>",true);
			...
			if ($session[user][acctid]==$row[marriedto] && $session[user][marriedto]==$row[acctid]){
				if ($session[user][charisma]==4294967295 && $row[charisma]==4294967295){
					output("`@`bDein".($session[user][sex]?" Mann":"e Frau")."!`b`n`n`n`0");
	...
	if (db_num_rows($result)>0){
		...
		//if ($session[user][lastip]==$row[lastip] || ($session[user][emailaddress]==$row[emailaddress] && $row[emailaddress])){
			...
			output("So gern du auch flirten möchtest, dein Partner geht dir nicht aus dem Kopf! Verwirrt drehst du wieder um.");
			
		} else if ( $row['charisma'] == 999 && $row['marriedto'] != $session['user']['acctid'] ) {
		
			output("Neidisch beobachtest du `6".$row['name']."`0 bei ".($row['sex'] ? 'ihren' : 'seinen')." Hochzeitsvorbereitungen. In diese traute Zweisamkeit willst du lieber nicht hereinplatzen..");
		
		} else if (($session[user][race]==2 && $row[race]==4) &&  ($row['marks']<31) && ($session['user']['marks']<31) || ($session[user][race]==4 && $row[race]==2)){
...
/*					output("`c`b`&Hochzeit!`0`b`c");
					...
					if($session['user']['charisma']!=999) {
						...
						output("`&Das heutige Treffen ist etwas Besonderes! Ihr versteht euch intuitiv besonders gut, scheint gar vor Liebe der Welt entrückt zu sein..`n"); 
						output("Nach einem langen Gespräch, in dem ihr euch immer wieder eurer Zuneigung versichert, fasst sich ".$row['name']."`& ein Herz und macht dir einen romantischen `bHeiratsantrag`b!`n`n");
						output("Ihr seid jetzt offiziell verlobt. In nächster Zeit wird ein Priester auf euch zukommen, um die Details eurer Hochzeit zu besprechen. Alternativ könntet natürlich auch ihr Kontakt mit den Priestern im Tempel aufnehmen!`n`n");					
						...
						output("`&Voller Vorfreude plant ihr eure Hochzeit. Wenn ihr nicht grade mit dem jeweils anderen beschäftigt seid..");
					}			
	...
	*/
	...
	output("Durch den Garten schlängelt sich ein silbrig glänzender, fröhlich plätschernder Fluss, über den sich eine Brücke aus weißem Stein spannt. Verborgen hinter einigen Bäumen liegt ein kleiner Tempel, der majestätische Gelassenheit ausstrahlt.");
